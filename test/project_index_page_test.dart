...
void main() {
  group('ProjectIndexPage with stores', () {
    late MockProjectStore mockProjectStore;
    ...
    setUp(() {
      ...
      when(mockProjectStore.getProjectById(1)).thenReturn(project);
      when(mockProjectStore.projects).thenReturn([project]);
      when(mockSessionStore.sessionsForProject(project.id!)).thenReturn([]);
      when(mockSessionStore.isLoading(project.id!)).thenReturn(false);
      when(mockSessionStore.errorForProject(project.id!)).thenReturn(null);
      when(mockSessionStore.hasSessions(project.id!)).thenReturn(false);
      when(mockSessionStore.loadSessions(project.id!)).thenAnswer((_) async {});
    });

    Widget createTestWidget(Widget child) {
      return MultiProvider(
        providers: [
          ChangeNotifierProvider<ProjectStore>.value(value: mockProjectStore),

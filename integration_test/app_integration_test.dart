...
void main() {
  ...
  group('End-to-end App Test', () {
    ...
    setUp(() {
      ...
      when(mockDbHelper.create(any))
          .thenAnswer((_) async => testProject);

      // Stubbing for ProjectIndexPage navigation and session display
      when(mockDbHelper.readProject(testProject.id!))
          .thenAnswer((_) async => testProject);
      when(mockDbHelper.readAllSessionsForProject(testProject.id!))
          .thenAnswer((_) async => []);
      ...
      when(mockDbHelper.updateSession(argThat(isA<Session>().having((s) => s.name, 'name', 'Evening Run'))))
          .thenAnswer((_) async => 1);

      // Stubbing for deleting a session
      when(mockDbHelper.deleteSession(testSession.id!))
          .thenAnswer((_) async => 1);

      // Stubbing for deleting a project
      when(mockDbHelper.delete(testProject.id!))
          .thenAnswer((_) async => 1);
    });
    ...
    testWidgets('Full app flow: add project, add session, rename session, delete session, delete project', (WidgetTester tester) async {
      app.main();
      ...
      await tester.pumpAndSettle();
      await tester.enterText(find.widgetWithText(TextField, 'Titre du projet'), 'My Awesome Project');
      ...
      expect(find.text('Morning Ride'), findsOneWidget); // Session name in AppBar of SessionCompletionPage
      expect(find.text('Go!'), findsOneWidget); // Button on SessionCompletionPage

      // Simulate stopping the session (this will pop back to ProjectIndexPage)
      await tester.tap(find.text('Go!')); // Tap to start recording
      await tester.pumpAndSettle(const Duration(seconds: 2)); // Simulate some recording time
      await tester.tap(find.text('Stop')); // Tap to stop recording
      await tester.pumpAndSettle();

      // Update stub for readAllSessionsForProject after adding a session
      when(mockDbHelper.readAllSessionsForProject(testProject.id!))
          .thenAnswer((_) async => [testSession.copy(duration: const Duration(seconds: 2), gpsPoints: 2)]);
      ...
      when(mockDbHelper.readAllSessionsForProject(testProject.id!))
          .thenAnswer((_) async => [testSession.copy(name: 'Evening Run', duration: const Duration(seconds: 2), gpsPoints: 2)]);
